...
//                    sh label: '', script: '''java -jar "pipeline-scan-LATEST/pipeline-scan.jar" -vid $veracode_id -vkey $veracode_key
//                                                       --fail_on_severity "High" --fail_on_cwe 80 --baseline_file verainsecurePipelineResults.json
//                                                      -f "target/verainsecure.war" -json_output_file '''
//                }
//            }
//        }
        ...
        stage("Veracode Static Analysis") {
            steps {
                withCredentials([usernamePassword(credentialsId: 'veracode-credentials', passwordVariable: 'veracode_key', usernameVariable: 'veracode_id')]) {
                    veracode applicationName: 'VeraInsecure',
                    criticality: 'Medium', debug: true,
                    scanName: 'Jenkins - Build $buildnumber',
                    uploadIncludesPattern: '**/**.war',
                    useIDkey: true, vid: env.veracode_id, vkey: env.veracode_key
                }
            }
        }
    }
